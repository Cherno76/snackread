...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // LOCAL PATCH (SnackRead): `Retained::as_ptr` hands back a pointer to an
      // object that is released when `config` drops, so UIKit's `objc_retain`
      // in `-[UIApplication _connectUISceneFromFBSScene:…]` segfaulted.
      // Return an autoreleased (+0) pointer, matching the ObjC convention.
      // Upstream fix: tauri-apps/tao f2163508 (0.36.0).
  ...
  unsafe {
    ...
    // LOCAL PATCH (SnackRead): always register the scene-connection callback.
    // iOS 26/27 kills apps that haven't adopted the UIScene life cycle, and
    // UIKit's adoption check is satisfied by this delegate method. Gating it on
    // `UIApplicationSupportsMultipleScenes` left single-scene apps (ours) with a
    // scene manifest but no callback, which traps at launch with
    // "_UIApplicationEvaluateRuntimeIssueForNoSceneLifecycleAdoption".
    // Upstream fix: tauri-apps/tao a3ff3f03 (0.37.0).
    decl.add_method(
