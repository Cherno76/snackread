...
///
/// LOCAL PATCH (SnackRead): upstream 1.0.8 only globalizes symbols belonging to
/// the package's *own* module. Xcode 27 also internalizes `@_cdecl` exports of
/// dependency modules bundled into the same archive — for Tauri that is the
/// `SwiftRs` module (`_string_from_bytes`, `_retain_object`, `_release_object`),
/// which then fails the Rust link with "Undefined symbols". Consider every
/// member of the archive; the plain-C-name and uniqueness filters above still
/// apply, so ld's duplicate-name assertion is not reintroduced.
